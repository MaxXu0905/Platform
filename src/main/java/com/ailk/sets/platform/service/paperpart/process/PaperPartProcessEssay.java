...
@Service
public class PaperPartProcessEssay extends PaperPartProcessAbstract implements IPaperPartProcess {

	private Logger logger = LoggerFactory.getLogger(PaperPartProcessEssay.class);
	@Override
	public void processPaperParts(PaperSet paperSet, List<QbQuestion> selfQbQuestions) {
        ...
        qbQuestionService.savePaperQuestions(questions, paper, PaperPartSeqEnum.ESSAY, totalTime);
    	//智力题增加选择题 试答题
		PaperPart paperPart = paperPartDao.getEntity(new PaperPartId(paper.getPaperId(), PaperPartSeqEnum.TEST_OBJECT
				.getValue()));// 是否包含测试试答题
